...
- 🖼️ **窗口比例锁定与铺满**：窗口模式下缩放窗口时实时维持视频宽高比，窗口恰好框住视频、无黑边无裁切；窗口与全屏下均可一键切换铺满（裁切填满）与信箱（保持比例）缩放。
- ⚙️ **多维度自定义设置**：
...
```powershell
powershell -ExecutionPolicy Bypass -File tools\build-release.ps1 -Version 1.2.1
```
打包成功后，将在 `publish\AirPlayer-1.2.1-win-x64\` 中生成免安装绿色版本（包含 exe 启动程序、所有依赖运行时以及自动嵌入的 `fdk-aac.dll`）。
